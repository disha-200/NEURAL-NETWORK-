...
def one_hot_encoding(onehot_data):
    # column_list = data_wo_target.columns.tolist()
    onehot_data = onehot_data.copy()
    # print(column_list[0])
    onehot_data['Gender'] = onehot_data['Gender'].map({'Male': 'cat0', 'Female': 'cat1'})
    one_hot_encode_first = pd.get_dummies(onehot_data['Gender'], prefix='Gender')
    # print(one_hot_encode_first)

    onehot_data['Married'] = onehot_data['Married'].map({'Yes': 'cat0', 'No': 'cat1'})
    one_hot_encode_second = pd.get_dummies(onehot_data['Married'], prefix='Married')
    # print(one_hot_encode_second)

    onehot_data['Dependents'] = onehot_data['Dependents'].map({'0': 'cat0', '1': 'cat1', '2': 'cat2', '3+': 'cat3'})
    one_hot_encode_third = pd.get_dummies(onehot_data['Dependents'], prefix='Dependents')
    # print(one_hot_encode_third)

    onehot_data['Education'] = onehot_data['Education'].map({'Graduate': 'cat0', 'Not Graduate': 'cat1'})
    one_hot_encode_fourth = pd.get_dummies(onehot_data['Education'], prefix='Education')
    # print(one_hot_encode_fourth)

    onehot_data['Self_Employed'] = onehot_data['Self_Employed'].map({'Yes': 'cat0', 'No': 'cat1'})
    one_hot_encode_fifth = pd.get_dummies(onehot_data['Self_Employed'], prefix='Self_Employed')
    # print(one_hot_encode_fifth)

    onehot_data['Credit_History'] = onehot_data['Credit_History'].map({0: 'cat0', 1: 'cat1'})
    one_hot_encode_sixth = pd.get_dummies(onehot_data['Credit_History'], prefix='Credit_History')
    # print(one_hot_encode_sixth)

    onehot_data['Property_Area'] = onehot_data['Property_Area'].map({'Rural': 'cat0', 'Urban': 'cat1',
                                                                     'Semiurban': 'cat2'})
    one_hot_encode_seventh = pd.get_dummies(onehot_data['Property_Area'], prefix='Property_Area')
    # print(one_hot_encode_seventh)

    final_one_hot = pd.concat([one_hot_encode_first, one_hot_encode_second, one_hot_encode_third, one_hot_encode_fourth,
                               one_hot_encode_fifth, one_hot_encode_sixth, one_hot_encode_seventh]
                              , axis=1)
...
# print(target_column)
data_wo_target = data.drop(['target', 'Loan_ID'],axis=1,inplace=False)

# print(data_wo_target['Credit_History'].unique())
one_hot_data = data_wo_target[['Gender', 'Married', 'Dependents', 'Education', 'Self_Employed',
                               'Credit_History', 'Property_Area']]
one_hot_data = one_hot_encoding(one_hot_data)

ndata = n(data_wo_target[['ApplicantIncome', 'CoapplicantIncome', 'LoanAmount', 'Loan_Amount_Term']])
# print(ndata)

data = pd.concat([one_hot_data, ndata, target_column], axis=1)
# print(data)
k = 10
...
for i in range(k):
    ...
    network.back_propogation(X_train, y_train, iterations=500, regularizer_lambda=0.0, alpha=1.0)
    # cost_output = network.cost_function(X_train, y_train, regularizer_lambda=0.00)
