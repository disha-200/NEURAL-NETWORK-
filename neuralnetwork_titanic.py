...
def one_hot_encoding(onehot_data):
    ...
    # print(column_list[0])
    onehot_data['Pclass'] = onehot_data['Pclass'].map({1: 'cat0', 2: 'cat1', 3: 'cat2'})
    one_hot_encode_first = pd.get_dummies(onehot_data['Pclass'], prefix='Pclass')
    # print(one_hot_encode_first)

    onehot_data['Sex'] = onehot_data['Sex'].map({'male': 'cat0', 'female': 'cat1'})
    one_hot_encode_second = pd.get_dummies(onehot_data['Sex'], prefix='Sex')
    # print(one_hot_encode_second)

    onehot_data['Siblings/Spouses Aboard'] = \
        onehot_data['Siblings/Spouses Aboard'].map({0: 'cat0', 1: 'cat1', 2: 'cat2', 3: 'cat3', 4: 'cat4', 5: 'cat5',
                                                    6: 'cat6', 7: 'cat7', 8: 'cat8'})

    one_hot_encode_third = pd.get_dummies(onehot_data['Siblings/Spouses Aboard'], prefix='Siblings/Spouses Aboard')
    # print(one_hot_encode_third)

    onehot_data['Parents/Children Aboard'] = \
        onehot_data['Parents/Children Aboard'].map({0: 'cat0', 1: 'cat1', 2: 'cat2', 3: 'cat3', 4: 'cat4', 5: 'cat5',
                                                    6: 'cat6'})

    one_hot_encode_fourth = pd.get_dummies(onehot_data['Parents/Children Aboard'], prefix='Parents/Children Aboard')
...
# print(target_column)
data_wo_target = data.drop(['target', 'Name'], axis=1, inplace=False)

one_hot_data = data_wo_target[['Pclass', 'Sex', 'Siblings/Spouses Aboard', 'Parents/Children Aboard']]
one_hot_data = one_hot_encoding(one_hot_data)
# print(one_hot_data)

ndata = n(data_wo_target[['Age', 'Fare']])
# print(ndata)
...
for i in range(k):
    ...
    network.back_propogation(X_train, y_train, iterations=1500, regularizer_lambda=0.25, alpha=0.75)
    # cost_output = network.cost_function(X_train, y_train, regularizer_lambda=0.00)
